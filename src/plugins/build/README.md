...
| `compile` | `project` + `tauri.build()` | iOS first calls `project.clearMobileBuildOutput({ target })` — `tauri ios build` renames its fresh `.app` into the archive the previous build left in `gen/apple/build` and fails the SECOND build of a tree with `Directory not empty`. It runs inside the compile phase, so a refusal from that guarded remove is reported as a compile error, and `tauri.build` never starts. Something removed → one `status: "progress"` event with detail `"cleared previous build output"`; nothing removed → silence. Desktop and Android skip it entirely. Then one subprocess covers compile+bundle (D-013); `compile` carries the live `onTick` progress stream (`status: "progress"`, real crate counts, never a fake percentage). |
